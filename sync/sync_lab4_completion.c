...
DECLARE_COMPLETION(data_read_complete);

static int wait_function(void *unused)
{
    while(1) {
        pr_info("LDDChar: Waiting for read completion...\n");
        wait_for_completion (&data_read_complete);
        pr_info("LDDChar: Read completion triggered!!!\n");
    }
    do_exit(0);
    return 0;
}
...
static int __init chardrv_init(void)
{
   ...
   if (wait_thread) {
           pr_info("LDDChar: Thread created successfully\n");
    ...
    unregister_chrdev(majorNumber, DEVICE_NAME);
	kfree(membuff);
	pr_err("LDDChar:  Thread creation failed\n");
   }
...
MODULE_DESCRIPTION("LDD: Module to demonstrate completions");
